...
public class ProjDiagram extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		if (v[4].isZero()) {
			...
			label = new String(v[4].isKnown() ? v[4].contents : v[4].name);
		...
		if (v[12].getVal() == 0 || !v[12].isKnown()) {
			...
			label = v[12].isKnown() ? v[12].contents : v[12].label;
		...
		if (!v[10].isZero()) {
			...
			MathUtil.drawArrow(g2d, rx, (int) (ry + 1.5 * r), (int) (widthDivByTen + r),
					(int) (ry + 1.5 * r), r);
			label = new String(v[10].isKnown() ? v[10].contents : v[10].label);
		...
		if (v[9].getVal() > 0 || !v[9].isKnown()) {
			...
			label = new String(v[0].isKnown() ? v[0].contents : v[0].label);
			g2d.drawString(label, (int) (widthDivByTen + 2.1 * r), (int) (y * 1.01));
			label = new String(v[9].isKnown() ? v[9].contents : v[9].label);
		...
		if (!v[8].isZero()) {
			...
			label = new String(v[8].isKnown() ? v[8].contents : v[8].label);
		...
		if (v[3].getVal() < 0 || !v[3].isKnown()) {
			...
			label = new String(v[3].isKnown() ? v[3].contents : v[3].label);
			g2d.drawString(label + " m/s", (int) (rx - r), (int) (ry + 2 * r));
		}
		if (!v[2].isZero()) {
			MathUtil.drawArrow(g2d, rx, ry + r / 2, rx + 3 * r, ry + r / 2, r);
			label = new String(v[2].isKnown() ? v[2].contents : v[2].label);
