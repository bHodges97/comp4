		...
		buOk.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!MathUtil.isNumeric(mass)){
					JOptionPane.showMessageDialog(getParent(), "Please enter a valid number(mass)!","Oops",JOptionPane.ERROR_MESSAGE);
					return;
				}
				if(!MathUtil.isNumeric(comX) || !MathUtil.isNumeric(comY)){
					JOptionPane.showMessageDialog(getParent(), "Please enter a valid number(fenter of mass)!","Oops",JOptionPane.ERROR_MESSAGE);
					return;
				}
				if(!MathUtil.isNumeric(PosX) || !MathUtil.isNumeric(PosY)){
					JOptionPane.showMessageDialog(getParent(), "Please enter a valid number(position)!","Oops",JOptionPane.ERROR_MESSAGE);
					return;
				}
